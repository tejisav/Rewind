...
angular.module('rewind.services').factory('rewindDB', function ($q, $cordovaSQLite, $cordovaFile, rewindConfig, imageService, onlineArtwork) {
    var database = {
        db: null,
        ...
        removeAllArtworks: function () {
            ...
            $cordovaSQLite.execute(this.db, "SELECT file_name FROM artwork WHERE id>1", []).then(function (res) {
                console.log('got all artworks');
                var promises = [];
                var artworkDir = rewindConfig.appRootStorage + 'artworks/';
                var miniaturesDir = rewindConfig.appRootStorage + 'miniatures/';
                for (var i = 0; i < res.rows.length; ++i) {
            ...
            $cordovaSQLite.execute(dbService.db, "SELECT id FROM title WHERE path=?", [title.path]).then(function (res) {
                if (res.rows.length === 0) {
                    dbService.addArtist(title.artist).then(function (artistId) {
                        ...
                        .then(function (res) {
                            if (title.artwork) {
                                $cordovaFile.removeFile(rewindConfig.appRootStorage + 'tmp/', title.artwork);
                            }
                            defered.resolve(res.insertId);
                        }, function (err) {
        ...
        addArtworkScan: function (fileName) {
            ...
            if (fileName) {
                self.addArtwork(fileName).then(function (insertId) {
                    ...
                    var dataPath = rewindConfig.appRootStorage;
        ...
        updateTitlePlayStatistics: function (titleId) {
            return $cordovaSQLite.execute(this.db, "UPDATE title SET nb_played = nb_played + 1, last_play = CURRENT_TIMESTAMP WHERE id=?", [titleId]);
        },
